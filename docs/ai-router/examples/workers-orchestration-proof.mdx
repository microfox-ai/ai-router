...
Consumes process output (e.g. from queue step 1) and produces a summary report. Used as queue step 2 with a `chain` function that maps the previous step's output.
...
- **Queue demo** (`/workflows/queue-demo`) – Runs queue **demo-data-processor**: step 1 = demo (process), step 2 = results-aggregator (with `delaySeconds` and a `chain` function mapping the previous output).
...
- **chain** functions and **delaySeconds** in queues,
