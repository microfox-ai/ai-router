...
### Step 2: Use the Agent as a Tool

Now you can use this agent as a tool in LLM functions or other agents:
...
const emailSenderAgent = new AiRouter();
...
dataAnalysisAgent
  ...
  .actAsTool('/', {
    ...
    metadata: { icon: '📊', category: 'analysis' },
  });
...
reportGeneratorAgent
  ...
  .actAsTool('/', {
    ...
    metadata: { icon: '📄', category: 'generation' },
  });
...
researchAgent
  ...
  .actAsTool('/', {
    ...
    metadata: { icon: '🔍', category: 'research' },
  });
...
summarizerAgent
  ...
  .actAsTool('/', {
    ...
    metadata: { icon: '📝', category: 'processing' },
  });
...
robustAgent
  ...
  .actAsTool('/', {
    ...
    metadata: { icon: '🛡️', category: 'processing' },
  });
```
...
writingAgent
  ...
  .actAsTool('/', {
    ...
    metadata: { icon: '✍️', category: 'writing' },
  });
