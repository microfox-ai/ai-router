...
type WorkerHandler<INPUT, OUTPUT> = (args: {
  ...
  ctx: {
    jobId: string;
    workerId: string;
    userId?: string;         // From DispatchOptions.userId
    jobStore?: JobStore;
    ...
    /** Report LLM token usage after each call. Throws TokenBudgetExceededError if maxTokens exceeded. */
    reportTokenUsage: (usage: { inputTokens: number; outputTokens: number }) => Promise<void>;
    /** Get current token usage and remaining budget for this job. */
    getTokenBudget: () => { used: number; budget: number | null; remaining: number | null };
    /** Populated on retry attempts (attempt ≥ 2). Undefined on first attempt. */
    retryContext?: {
      attempt: number;
      maxAttempts: number;
...
- `ctx.reportTokenUsage({ inputTokens, outputTokens })` accumulates LLM usage and throws `TokenBudgetExceededError` when `maxTokens` is exceeded. See [Token Budget](#token-budget) below.
- `ctx.retryContext` is populated on retries — use it to inject the previous error into your next prompt for self-correction. See [Smart Retry](#smart-retry) below.

## Smart Retry

Workers support **in-process smart retry**: when the handler throws a matching error, the runtime retries it in the same Lambda invocation (no SQS re-delivery, job stays `running`). On each retry, `ctx.retryContext` is populated so the handler can self-correct.

### Configuration

Pass a `retry` config to `createWorker`:
...
export default createWorker({
  ...
  // Retry up to 3 times on rate-limit or JSON parse errors
  retry: {
    maxAttempts: 3,
  ...
  handler: async ({ input, ctx }) => {
    // On retries, ctx.retryContext is populated:
    if (ctx.retryContext) {
      ctx.logger.warn(`Retry attempt ${ctx.retryContext.attempt}/${ctx.retryContext.maxAttempts}`, {
        error: ctx.retryContext.lastError.message,
      });
    }
    // ... handler logic
  },
...
| Pattern | Matches | Delay |
|---------|---------|-------|
| `'rate-limit'` | HTTP 429, `RateLimitError`, "rate limit" in message | 10s, 20s, 30s… |
| `'json-parse'` | `SyntaxError`, `ZodError`, "json/parse" in message | Immediate (injects `ctx.retryContext`) |
| `'overloaded'` | HTTP 529, "overloaded/model is busy" in message | 15s, 30s… |
| `'server-error'` | HTTP 5xx, "internal server error/service unavailable" | 5s, 10s… |
...
retry: {
  ...
  on: [
    'rate-limit',               // built-in
    {
      // Custom: retry on any validation error
      match: /validation failed/i,
      delayMs: 2000,
      injectContext: true,      // populate ctx.retryContext.lastError
    },
    {
      // Custom: retry based on error properties
      match: (err) => err.code === 'ECONNRESET',
      delayMs: (attempt) => attempt * 3000,  // progressive delay
    },
  ],
},
```

### Self-correction pattern (LLM workers)

```typescript
handler: async ({ input, ctx }) => {
  // Inject previous error into the prompt for self-correction
  const errorHint = ctx.retryContext
    ? `\n\nPrevious attempt failed: "${ctx.retryContext.lastError.message}". Fix your response.`
    : '';
  ...
  // Throws SyntaxError → matches 'json-parse' → triggers retry with ctx.retryContext
  return JSON.parse(response.content[0].text);
},
```

### Per-queue-step retry

A queue step's `retry` overrides the worker-level retry for that step only:
...
defineWorkerQueue({
  ...
  steps: [
    { workerId: 'fast-worker' },
    {
      ...
      retry: { maxAttempts: 5, on: ['rate-limit', 'json-parse'] },
    },
  ],
});
```

> **Note**: `TokenBudgetExceededError` is **never retried**, regardless of which patterns are configured.

## Token Budget

The token budget feature lets you cap LLM usage per job dispatch. Pass `maxTokens` in `DispatchOptions`; the worker calls `ctx.reportTokenUsage()` after each LLM call and the runtime throws `TokenBudgetExceededError` (which terminates the job cleanly, never retried) when the total exceeds the limit.

### Basic usage

```typescript
// Dispatch with a token budget
await worker.dispatch(input, {
  maxTokens: 50_000,   // total input + output tokens allowed for this job
  metadata: { source: 'api' },
});
```

```typescript
// Inside the handler: report usage after each LLM call
handler: async ({ input, ctx }) => {
  const response = await anthropic.messages.create({ /* ... */ });

  // Accumulates usage — throws TokenBudgetExceededError if limit exceeded
  await ctx.reportTokenUsage({
    inputTokens: response.usage.input_tokens,
    outputTokens: response.usage.output_tokens,
  });

  // Check remaining budget before making more calls
  const budget = ctx.getTokenBudget();
  if (budget.remaining !== null && budget.remaining < 1000) {
    ctx.logger.warn('Low token budget, stopping early', { remaining: budget.remaining });
    return earlyResult();
  }
...
Token usage is persisted to the job store after each `reportTokenUsage()` call, stored as `metadata.tokenUsage: { inputTokens, outputTokens, budget }`. You can read it from the job record via `GET /api/workflows/workers/:workerId/:jobId`.

### With smart retry

`TokenBudgetExceededError` is never retried — even if you have `'server-error'` or other patterns configured, the job will fail immediately when the budget is exceeded:

### Lambda deployment (WorkerConfig)

When you compile workers with `npx ai-worker compile` (or deploy with `npx microfox deploy`), the CLI generates Lambda functions and SQS queues from your worker definitions. You can customize each worker’s Lambda and queue behavior by exporting a **`WorkerConfig`** from the same file as the worker.
...
1. **Definition** – Queues are defined in `.queue.ts` files with an `id` and a `steps` array. Each step has: `workerId` (required), `delaySeconds?`, `chain?` (input mapping), `resume?` (HITL merge), `requiresApproval?` (pause for human review), `hitl?` (UI metadata), and `loop?` (repeat logic).
...
- **POST /api/workflows/queues/:queueId/approve** – HITL: approve or reject a paused step (body: `queueJobId`, `decision`, `input?`, `comment?`, `reviewerId?`)

Optional app logic (e.g. cooldown) can list recent jobs for a queue via the same API or the job store before triggering.

### Human-in-the-loop (HITL) queue steps

A queue step with `requiresApproval: true` pauses after the previous step completes. The queue job status becomes `awaiting_approval` and polling stops advancing until a human submits a decision. Add a `chain` function to shape the input before the pause, a `resume` function to merge the reviewer's payload when resumed, and a `hitl` config for UI metadata. Use `useWorkflowJob`'s `hitlTask` and `submitHitlDecision` helpers on the frontend.
...
export default defineWorkerQueue({
  ...
  steps: [
    { workerId: 'prepare-data' },
    {
      workerId: 'review-step',
      requiresApproval: true,
      chain: 'passthrough',
      resume: (ctx) => ({ ...ctx.pendingInput, decision: ctx.reviewerInput }),
      hitl: defineHitlConfig({
        taskKey: 'data-review',
        ui: { type: 'schema-form', title: 'Review the prepared data' },
...
See [HITL Queues](../advanced/hitl-queues) for the full guide, including looping steps, custom view components, and the approve endpoint contract.
...
**Options (both worker and queue):** `baseUrl`, `pollIntervalMs`, `pollTimeoutMs`, `autoPoll`, `onComplete`, `onError`, `enabled`. **Returned:** `trigger(input?)`, `jobId`, `status` (`idle` | `queued` | `running` | `awaiting_approval` | `completed` | `failed` | `partial`), `output`, `error`, `loading`, `polling`, `reset()`.

**Queue-only HITL helpers:**
- `hitlTask` – derived `QueueHitlTask` when a step is `awaiting_approval`; has `taskId`, `queueJobId`, `stepIndex`, `workerId`, `progress`, `previousOutputs`, `uiSpec`, `inputSchema`
- `submitHitlDecision(payload)` – POSTs `{ decision, input?, comment?, reviewerId? }` to the approve endpoint; `decision` is `'approve'` or `'reject'`

When a step is paused, `status === 'awaiting_approval'`, `polling` remains `true`, and the poll timeout resets to give the reviewer unlimited time (the timeout only counts active processing time).
...
You can also run a full local worker stack with `npx ai-worker dev` — an HTTP server on `localhost:4100` exposing the same routes as a deployed core group (trigger, config, queue start), with in-process worker execution, hot reload, and a file-persisted job store. Point your app at it with `WORKER_BASE_URL=http://localhost:4100`. See the [CLI reference](../api-reference/ai-worker-cli#dev-local-dev-server) for details.
...
See the [CLI reference](../api-reference/ai-worker-cli) for deployment details. `npx microfox deploy` compiles and pushes in one step; `npx ai-worker compile` (or `npx microfox compile`) builds only, and `npx microfox push` uploads an existing build. Add `--stage <stage>` for parallel staging/prod stacks.

**Deploy parameters (multi-group):** When using multiple groups, you can deploy a single group or skip groups: `npx microfox deploy [groupname]` compiles and deploys only that group; `npx microfox deploy --skip-group "g1,g2,g3"` deploys all groups except the listed ones. The same selection works on `microfox compile` and `microfox push` separately.

**Cross-group dispatch:** Same-group workers get `WORKER_QUEUE_URL_*` and IAM automatically. For queue chains, the next-step worker's queue URL and IAM are injected even when that worker is in another group. For ad-hoc worker-to-worker across groups, add `WORKER_QUEUE_URL_<SANITIZED_ID>` and IAM in that stack's serverless.yml.

**Project config (microfox.json):** Optional `worker` section in the project root configures bundling and per-group behavior: `externalDeps` (packages to keep external), `includeNodeModules` (bundle node_modules or not), and `groups` (e.g. `scraper: { includeNodeModules: true }` for heavy groups like Puppeteer). See [Worker Architecture – Project config](../advanced/worker-architecture#project-config-microfoxjson) for the full shape and examples.
