...
**Push parameters (multi-group):** When using multiple groups, you can deploy a single group or skip groups: `npx microfox push [groupname]` deploys only that group; `npx microfox push --skip-group "g1,g2,g3"` deploys all groups except the listed ones. From the ai-worker CLI, `npx ai-worker push --skip-core` deploys every group except core (it runs `npx microfox push --skip-group "core"`).
