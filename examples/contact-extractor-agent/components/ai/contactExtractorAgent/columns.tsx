...
async function analyzePersona(contactId: string, urls: string[]) {
  const response = await fetch(
    `/api/studio/chat/agent/extract/deep-persona?contactId=${contactId}&urls=${urls}`,
  );
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const handleAnalyze = async () => {
        ...
        const urls = Object.values(contact.socials || {}).filter(
          Boolean,
        ) as string[];
        ...
        <div className="flex space-x-2">
          {contact.persona && (
            <Button
              ...
              onClick={() => openPersonaModal(contact)}
            >
