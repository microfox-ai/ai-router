  ...
  .agent('/', async (props) => {
    ...
    const stream = streamText({
      ...
      system: dedent`
          ...
          If the user asks a question or a query, 
           - first fetch the documents that would be of help from the knowledgebase
           - then call the related generateTool to answer the user's question.
          If the user submits information ( link , text, file, etc.. ),
           - fetch information from the link if its a link.
           - save it to the knowledgebase.
        `,
