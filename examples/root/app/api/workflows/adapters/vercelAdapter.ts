...
export const vercelWorkflowAdapter: WorkflowRuntimeAdapter = {
  ...
  async resumeWebhook<Payload, Output>(
    ...
    // Vercel workflow API: resumeWebhook expects (token, request: Request)
    // Since we only have a payload, we construct a minimal Request object
    // or fallback to resumeHook which accepts payload directly
    try {
      // Construct a Request object from the payload for resumeWebhook
      // This is a workaround since resumeWebhook expects a Request object
      const requestBody = JSON.stringify(payload);
      const request = new Request('https://workflow.webhook', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: requestBody,
      });

      // Try resumeWebhook with constructed Request
      await (resumeWebhook as any)(token, request);

      return {
        status: 'resumed',
      };
    } catch (error: any) {
      // If resumeWebhook fails (e.g., wrong signature), fallback to resumeHook
      try {
        await resumeHook(token, payload);
        return {
          status: 'resumed',
        ...
        throw new Error(
          `[ai-router][workflow] Failed to resume webhook with token ${token}: ${error?.message || hookError?.message || String(error)}`,
        );
      }
    }
  },
