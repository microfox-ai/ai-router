      ...
      try {
        ...
        if (response.ok) {
          ...
          if (result && result.status) {
            ...
            if (result.hook?.token) {
              setHookToken(result.hook.token);
            }
                  ...
                  <div className="mt-2">
                    ...
                    <div className="flex gap-2">
                      <Button
                        size="sm"
                        onClick={async () => {
                          // Use hook token from status response if available, otherwise construct it
                          const token = hookToken || `research-approval:${topic}:${email}`;
                          await fetch(`/api/studio/chat/agent/workflows/research/signal`, {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({
                              token,
